...
- 📄 **多格式试卷智能拆解**：支持直接拖入 **LaTeX 源码 (.tex)**、**PDF 试卷 (.pdf)** 或 **Word 试卷 (.docx)** 快速智能切片拆题，自动提取原画级配图与原生公式，自带难度标签与变式题/子母题关联管理。
...
- **📄 Word (.docx) 试卷原生直提支持**：系统内置纯 Python 实现的 OMML (Office / MathType 数学公式流) 与 DrawingML 图片/表格提取引擎。支持直接拖放 `.docx` 试卷文件，无需安装 Microsoft Word 软件即可 0 损直提原生 LaTeX 公式、原画级插图与结构化表格，并由大模型自动切片拆题。
