...
1. **自定义大纲与难度/题型配置**：
   - 点击网页右上角的 **「设置」** 按钮，切换到 **「维度配置」** 选项卡。
   - 您可以直接在线编辑 JSON 配置，自定义学段级联（如 `高中物理`、`初中数学`）、课程大纲（章节与小节）、题型（如 `单选题`、`简答题`）以及难度级（如 `入门`、`进阶`、`困难`，含小徽章配色样式）。
   - 编辑完成后点击保存，系统将自动重载数据库分类级联、AI 智能分类规则，并自动防错自愈。
