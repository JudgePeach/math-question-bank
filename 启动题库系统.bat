@echo off
:: 设置活动代码页为 UTF-8 以便在 Windows 命令行中正确显示中文
chcp 65001 >nul
cd /d "%~dp0"
...
if "%PYTHON_CMD%"=="" (
    echo ❌ 未检测到系统安装了 Python 环境，或者未将 Python 添加至环境变量 (PATH) 中！
...
if errorlevel 1 (
    ...
    echo ⚠️ 未检测到 curl 工具，将等待 3 秒后直接启动浏览器...
...
set HTTP_STATUS=000
for /f "delims=" %%i in ('curl -s -o nul -w "%%{http_code}" http://127.0.0.1:8000/api/questions 2^>nul') do set HTTP_STATUS=%%i
