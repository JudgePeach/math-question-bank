...
- **插图排版位置联动与互动切换 (Figure Placement Sync & Interactive Toggle)**：
  ...
  - **PDF 编译精准反馈**：后端 `build_latex_document` 编译试卷时读取 `figure_align`，`right` 采用顶部对齐的 `minipage` 双栏（配合 `\vspace{-2.0em}` 实现顶端精准对齐），`center` 与 `bottom_right` 采用对应的 `center` / `flushright` LaTeX 环境。
